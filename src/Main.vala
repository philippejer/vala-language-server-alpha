...
void main(string[] args)
{
  ...
  loglevel = LogLevel.from_json(Environment.get_variable("VLS_DEBUG"));
  logdebug = (loglevel >= LogLevel.DEBUG);
  loginfo = (loglevel >= LogLevel.INFO);
  logwarn = (loglevel >= LogLevel.WARN);
  method_completion_mode = MethodCompletionMode.from_json(Environment.get_variable("VLS_METHOD_COMPLETION_MODE"));
